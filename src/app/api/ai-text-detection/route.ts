...
import { AITextDetectionRequest, postAITextDetectionQuery } from './handlers'

export const maxDuration = 30

export async function POST(request: NextRequest) {
  const data = (await request.json()) as AITextDetectionRequest
  ...
  try {
    const result = await postAITextDetectionQuery(data)
