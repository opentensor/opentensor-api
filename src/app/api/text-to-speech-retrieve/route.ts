...
import { postTextToSpeechRetrieveResult, textToSpeechRetrieveRequest } from './handlers'

export const maxDuration = 30

export async function POST(request: NextRequest) {
  const data = (await request.json()) as textToSpeechRetrieveRequest
  ...
  try {
    const result = await postTextToSpeechRetrieveResult(data)
