...
import { postTextToSpeechCreate, textToSpeechCreateRequest } from './handlers'

export const maxDuration = 30

export async function POST(request: NextRequest) {
  const data = (await request.json()) as textToSpeechCreateRequest
  ...
  try {
    const result = await postTextToSpeechCreate(data)
